...
if (isset($_POST['submitcust'])){
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
     ...
     $sql = "SELECT eumob FROM end_user WHERE email='$email' AND euid!=$user_id AND status=1";
	 $query = $conn->query($sql);
	 $count = $query->num_rows;
      if ($count >0){
        $error="Profile Email Id Is Already Exist!";
        $show="display:show;";
        $alert="alert alert-danger";		
      }
	  else{
		$sql = "UPDATE end_user SET euname='$euname', eumob='$eumob', email='$email' WHERE euid=$user_id";
